  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!target) {
          send('error', { message: 'Message not found', reason: 'not_found' });
          controller.close();
          return;
        }
        if (target.role !== 'assistant') {
          send('error', { message: 'Can only regenerate assistant messages', reason: 'invalid_role' });
          controller.close();
          return;
        }

        // Determine new branch index for the sibling
        ...
        // Build message context: walk active path from root to the parent of target
        ...
        // Find the parent in the active path and slice up to (but not including) the target
        const parentId = target.parentMessageId;
        const parentIndex = parentId ? activePath.findIndex((m) => m.id === parentId) : -1;
        const historyPath = parentId
          ? activePath.slice(0, parentIndex + 1)
          : activePath.slice(0, 0);
        ...
        // Create new sibling branch message
        const newAssistantMsg = await prisma.message.create({
          data: {
            chatId,
            role: 'assistant',
            content: '',
        ...
        newAssistantMsgId = newAssistantMsg.id;

        // Update activeBranchesJson to point to the new branch
