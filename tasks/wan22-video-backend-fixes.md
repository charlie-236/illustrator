...
Verify byte-for-byte against `tasks/wan22_t2v.json` node 7's `inputs.text` — copy from the JSON, don't retype. Both templates use the same string; one source of truth.
