...
Projects are general containers for generated outputs. Today they hold images, video clips, and storyboards (relational Storyboard rows, Phase 5). The two-mode entry on the project detail view is the current shape; new entry points will be added as new generation surfaces ship.
...
**Storyboards are relational (updated in Phase 5d).** A project has zero or more `Storyboard` rows in the `Storyboard` table (see schema below). Each storyboard stores its scenes as `scenesJson` (a `StoryboardScene[]`). The old `Project.storyboardJson` column was removed.

**API:**
- `POST /api/storyboard/generate` — generates a storyboard (stateless; doesn't persist). Returns `{ ok: true, storyboard }` or `{ ok: false, reason }` with rawOutput for parse_error. Always HTTP 200 on success path; 400 for invalid_input/project_not_found.
- `POST /api/projects/[id]/storyboards` — creates a new empty `Storyboard` row for the project.
- `PUT /api/storyboards/[id]` — saves (replaces) a storyboard's scenes, name, storyIdea, generatedAt, quickGenerate.
- `DELETE /api/storyboards/[id]` — deletes a storyboard row; clips that referenced its scenes remain (sceneId becomes orphaned).
- `POST /api/storyboards/[id]/reorder` — repositions a storyboard among its siblings (`{ position: number }`).
...
**UI:** Project Detail has a storyboard tab strip above the Generate buttons. Empty project shows a "Plan with AI" entry point; once storyboards exist, tabs let the user switch between them with a "+" tab to create more.
...
- `src/app/api/projects/[id]/storyboards/route.ts` — POST creates a new storyboard row.
- `src/app/api/storyboards/[id]/route.ts` — PUT (save) and DELETE (delete) for individual storyboards.
- `src/app/api/storyboards/[id]/reorder/route.ts` — POST repositions a storyboard among siblings.
...
Bottom-sheet for editing a scene's `description`, `positivePrompt`, `durationSeconds` (stepper ±1, range 1–10), and `notes`. Tracks dirty state; shows discard confirm on close if unsaved. On save, PUTs the full updated storyboard to `/api/storyboards/[id]`. Also supports insert mode (scene is null + insertAtPosition provided) — creates a new scene with `crypto.randomUUID()` and splices it in at the given index.

### `CanonicalClipPickerModal` (new component)

Bottom-sheet listing all clips with `sceneId === scene.id`. Shows a "Canonical" badge on the current canonical. "Set as canonical" button PUTs updated storyboard to `/api/storyboards/[id]`. Tapping a clip tile opens `ImageModal` for detail view (mounted inside the picker at a higher z-index). Also exposes an "Attach existing clip" section (added in Phase 5d) for picking a clip from the project or from the gallery and writing its `sceneId` via `PATCH /api/generations/[id]/scene`.
...
The toggle defaults OFF — preserves Phase 5b's Studio-bounce behavior for users who want full parameter control. Toggle is per-storyboard (stored in `quickGenerate` on the `Storyboard` row), not per-project or per-scene.

**Toggle UI:** rendered in the storyboard section header (right side), a labeled switch with amber accent when active. Tap target ≥48px. Persists via PUT to `/api/storyboards/[id]` on each toggle.
...
- `validateStoryboard()` in `src/app/api/storyboards/[id]/route.ts` accepts the new field.
...
## Phase 5d — Multi-storyboard + scene management

Migrates storyboards from a single `Project.storyboardJson` column to a relational `Storyboard` table, and adds multi-storyboard tabs, scene reordering, compact view, clip attachment, and canonical play/stitch.
...
// Project.storyboardJson column REMOVED; Project gains:
storyboards Storyboard[]
```

`onDelete: Cascade` — deleting a project deletes all its storyboards. Clips that reference scene IDs from deleted storyboards keep their `sceneId` value (soft reference; becomes orphaned).

One-shot migration script: `scripts/migrate-storyboard-to-relational.ts`. Reads old `storyboardJson` via `$queryRaw` (before column drop), creates `Storyboard` rows, idempotent.

### API changes

| Route | Description |
|---|---|
| `POST /api/projects/[id]/storyboards` | Create a new empty storyboard. Auto-names it "Storyboard N"; position = MAX+1. Returns `{ storyboard: Storyboard }`. |
| `PUT /api/storyboards/[id]` | Save a storyboard (name, scenesJson, storyIdea, generatedAt, quickGenerate). Validates name 1–100 chars, scenes 0–50. Returns `{ storyboard: Storyboard }`. |
| `DELETE /api/storyboards/[id]` | Delete a storyboard row. Returns `{ ok: true }`. |
| `POST /api/storyboards/[id]/reorder` | Reposition a storyboard among siblings. Body: `{ position: number }`. Recomputes sequential positions of all siblings in a transaction. |
| `PATCH /api/generations/[id]/scene` | Set or clear `Generation.sceneId`. Body: `{ sceneId: string \| null }`. Used when attaching/detaching clips from scenes. |

`GET /api/projects/[id]` now returns `storyboards: Storyboard[]` (ordered by `position ASC`) instead of `storyboard: Storyboard | null`.

### Multi-storyboard tab strip (ProjectDetail)
...
### Insert scenes at position

Between each pair of adjacent scenes (in non-compact mode), an insert affordance ("+") appears. Clicking it opens `SceneEditModal` in insert mode with `insertAtPosition` set. Insert mode creates a new scene via `crypto.randomUUID()`, splices it in, renumbers `position`, and PUTs the full updated storyboard.

In compact mode, an "Add scene" button at the bottom of the list triggers insert at the end.

### Drag-to-reorder scenes

Scene list is wrapped in a `DndContext` + `SortableContext` with `verticalListSortingStrategy`. Each scene card is a `SortableSceneCard` using `useSortable`. On drag-end, scene order is recomputed in local state and PUT to `/api/storyboards/[id]`.

### Compact toggle

A compact/expanded toggle (per storyboard, stored in `sessionStorage` as `storyboard-compact-${storyboardId}`) switches between:
- **Expanded** (default): full scene card with all fields, insert affordances between scenes, drag handles.
- **Compact**: description-only rows, each with a spot-expand chevron for per-scene detail. Insert affordances hidden; drag-to-reorder still available via drag handle.

### Attach existing clip to scene (CanonicalClipPickerModal)

The `CanonicalClipPickerModal` gains an "Attach existing clip" section with two pickers:
- **From project**: grid of all project video clips (any `sceneId`). Selecting one calls `PATCH /api/generations/[id]/scene` to set `sceneId`, then PUTs the storyboard to update `canonicalClipId`.
- **From gallery**: inline paginated gallery (`mediaType=video`) for picking a clip that isn't yet in the project. Same attach flow.
- **Detach**: clears `canonicalClipId` from the storyboard's scene; does NOT clear `Generation.sceneId`.

### Play canonical scenes

When ≥ 2 scenes have a resolved canonical clip, a "▶ Play canonical" button appears in the storyboard section header. Clicking it enters a canonical-play mode: a single `<video>` element plays each canonical clip in scene order. Scene-index chips allow jumping. "Play again" at end resets. Mirrors the project-level play-through (Phase 2.2) but scoped to canonical clips in scene order rather than all project clips.

### Stitch canonical scenes

When ≥ 2 scenes have a resolved canonical clip, a "Stitch canonical" button also appears. Clicking it opens `StitchModal` pre-populated with the canonical clip IDs in scene order (`initialClipIds` prop). The user can still adjust selection and transition before stitching.

`StitchModal` accepts an optional `initialClipIds?: string[]` prop. When provided, checkboxes are pre-checked for those clip IDs (in the provided order) and unchecked for all others.

### Source layout additions (Phase 5d)

- `prisma/schema.prisma` — `Storyboard` model added; `Project.storyboardJson` removed.
- `scripts/migrate-storyboard-to-relational.ts` — one-shot migration.
- `src/types/index.ts` — `Storyboard` is now a relational row (has `id`, `projectId`, `name`, `position`); `ProjectDetail.storyboards: Storyboard[]`.
- `src/app/api/projects/[id]/storyboards/route.ts` — POST create storyboard.
- `src/app/api/storyboards/[id]/route.ts` — PUT save, DELETE delete.
- `src/app/api/storyboards/[id]/reorder/route.ts` — POST reposition.
- `src/app/api/generations/[id]/scene/route.ts` — PATCH set/clear sceneId.
- `src/components/SceneEditModal.tsx` — updated with insert mode.
- `src/components/CanonicalClipPickerModal.tsx` — updated with attach-existing-clip section.
- `src/components/StoryboardGenerationModal.tsx` — updated with `targetStoryboardId` prop for create-vs-update.
- `src/components/ProjectDetail.tsx` — multi-storyboard tab strip, compact toggle, drag-to-reorder scenes, insert affordances, play canonical, stitch canonical, `SortableSceneCard` module-level component.
